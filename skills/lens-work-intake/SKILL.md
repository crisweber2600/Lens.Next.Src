...
description: Starts durable Lens work units. Use when the user requests 'lens work intake', 'start work', or 'create related feature'.
---
...
This skill creates a durable unit of work by turning an idea, change request, or related feature into an inspectable Lens feature/work archive. Act as a work-intake steward: preserve the user's raw intent as concise file memory, connect related work without copying noise, and hand the unit to the Lens/BMad lifecycle with a clear next workflow.
...
A unit of work is the permanent historical record for one feature, change, investigation, or related follow-up. Create or update it under `{work_intake_path}/<work-id>/`. The work archive is authored truth for delivery history; living service/domain/program ledgers remain current operational truth and are only changed later by `lens-ledger-promotion`.
...
When Lens context is detected or supplied, also record `lens_feature_id`, `lens_track`, `lens_phase`, `lens_docs_path`, `lens_governance_repo_path`, `lens_feature_yaml_path`, `lens_constitution_status`, and `lens_preflight_status` as provenance fields. Do not create or edit Lens governance `feature.yaml`; route Lens feature creation and phase advancement through `lens-start`, `lens-init-feature`, `lens-next`, or the active Lens wrapper.
...
Write `links.md` with all known artifact paths and relationship metadata. If the work is ready for implementation, include a story-template note instructing `bmad-create-story` to cite this work archive under the story's Lens work-unit section.
...
In Lens mode, never write the governance repo directly. Lens intake records observed Lens context only; Lens lifecycle commands remain the authority for feature creation and phase movement.