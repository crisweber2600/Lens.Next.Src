---
title: 'Lens Reporting UI MVP1 Plan'
status: 'planned'
module_code: 'lens'
created: '2026-05-21'
---

# Lens Reporting UI MVP1 Plan

## Boundary

This repository now owns the reporting data contract through `lens-reporting-snapshot` and `assets/reporting-snapshot.schema.json`. A full web UI, authentication layer, refresh scheduler, and deployment target are deferred until an application host is identified.
...
The UI consumes `snapshot-{date}.json` from `lens-reporting-snapshot`. The schema must remain backward-compatible once a UI host is built. Source docs remain authored truth; UI state is read-only and time-bound.
