...
## Optional Lens Context Fields

These fields let Lens consume Lens context without becoming a Lens conductor. They are optional in standalone Lens projects. When populated by a Lens wrapper or local context detector, they are provenance fields only; authoritative Lens lifecycle state still lives in governance `feature.yaml`.
...
| `lens_feature_yaml_path` | optional | Local path or repo-relative path to the authoritative governance `feature.yaml`. |
