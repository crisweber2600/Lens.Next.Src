...
description: Sets up the LENS module in a project. Use when the user requests to install LENS, configure LENS, or setup LENS.
---
...
For a complete starter LENS artifact tree, run:

```bash
python3 ./assets/lens/scripts/lens_artifact_ops.py init --project-root "{project-root}"
```

This creates the Work Archive, Living Landscape, Derived Map, BMAD bridge, implementation guard, validation, Salmon, Auspex, and gate directories. It also creates `_bmad-output/project-context.md` if one does not already exist. Do not overwrite an existing project context without user approval.
