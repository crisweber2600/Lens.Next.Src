---
name: bmad-lens-auspex
description: Produce read-only stakeholder visibility from the Derived Map for status, risks, blockers, evidence, and BMAD progress.
---
...
Creates Auspex status files and summaries from graph projections; Auspex is never source truth.
...
1. Read the Derived Map and warnings.
2. Summarize system status, outcomes, journeys, slices, freshness, decisions, risks, blockers, BMAD progress, validation evidence, Salmon signals, and traceability.
3. Write read-only status artifacts.
4. Do not edit archive or landscape source files.
...
status.yaml, status.json, stakeholder-summary.md, optional dashboard.html.
