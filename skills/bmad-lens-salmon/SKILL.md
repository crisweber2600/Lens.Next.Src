---
name: bmad-lens-salmon
description: Raise and propagate upstream correction signals when implementation reveals reality that changes assumptions or scope.
---
...
Detects downstream truth from stories and implementation evidence, then decides whether LENS landscape and BMAD correct-course are needed.
...
1. Capture the downstream discovery.
2. Identify impacted slice, journey, outcome, capability, domain, program, or system nodes.
3. Assess severity and whether the issue is local or upstream.
4. Recommend local note, landscape update, validation update, or BMAD correct-course.
5. Record propagation and sync status.
...
salmon signal, propagation report, decision record.
